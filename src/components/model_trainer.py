...
class ModelTrainer:
    def __init__(self):
        self.model_training_config=ModelTraningConfig

    
    def initiate_model_trainer(self, train_array, test_array):
        try:
            logging.info("split training and test input dta")

            X_train, y_train, X_test, y_test = (
                train_array[:,:-1],  ## take out the last column 
                train_array[:, -1],  ## take only last column
                test_array[:,:-1],
                test_array[:, -1]
            )
            ...
            if best_model_score < 0.6:
                raise CustomException ("No model score is more than 60%")
